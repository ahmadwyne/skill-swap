                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          {/* Ensure that s.userId1 and s.userId1.name are not null */}
                          <div className="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 text-sm font-semibold">
                            {s.userId1?.name
                              ? s.userId1.name.split(' ').map(n => n[0]).join('')
                              : 'N/A'} {/* Fallback for missing name */}
                          </div>
                          <span className="text-base font-semibold text-gray-800">
                            {s.userId1?.name || 'Unknown'} {/* Fallback to 'Unknown' if name is not available */}
                          </span>
                        </div>
